...
export const PanelBase = ({ title, icon, children, className = "", rightAction }: any) => (
  <div className={`
    bg-white dark:bg-stone-800 
    rounded-3xl 
    shadow-xl shadow-stone-200/50 dark:shadow-black/50
    ...
    ${className}
  `}>
    <div className="bg-stone-50 dark:bg-stone-800/50 border-b border-stone-100 dark:border-stone-700 px-6 py-4 flex justify-between items-center backdrop-blur-sm">
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#ea580c] to-[#c2410c] flex items-center justify-center shadow-lg shadow-orange-500/30 text-white">
          <i className={`fas fa-${icon} text-sm`}></i>
        </div>
        <h2 className="font-black text-stone-800 dark:text-white uppercase tracking-wider text-xs">{title}</h2>
      </div>
      {rightAction}
    </div>
    <div className="p-6">
...
const MeatEditor = ({ recipe, setRecipe, onClose }: any) => {
  ...
  return (
    <PanelBase title="Customizar Carnes" icon="pen" className="fixed inset-x-0 top-[64px] bottom-0 z-[50] flex items-start justify-center p-3 bg-stone-50/95 dark:bg-stone-900/95 backdrop-blur-sm overflow-y-auto animate-in slide-in-from-bottom-4 duration-300 no-print" rightAction={
      <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600 transition active:scale-90">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          {localMeats.map((m: any, idx: number) => (
            <div key={idx} className="flex gap-2 items-end bg-stone-50 dark:bg-stone-700 p-2 rounded-lg border border-stone-100 dark:border-stone-600">
              <div className="flex-1 min-w-0">
                <label className="text-[8px] font-black text-stone-400 uppercase mb-0.5 block tracking-wider">Corte</label>
                <input type="text" value={m.name} onChange={(e) => updateMeat(idx, 'name', e.target.value)} className="w-full bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 rounded-md p-1.5 font-bold text-xs text-stone-800 dark:text-white outline-none focus:border-[#ea580c] transition-all" />
              </div>
              <div className="w-[70px] flex-shrink-0">
                <label className="text-[8px] font-black text-stone-400 uppercase mb-0.5 block tracking-wider text-center">%</label>
                <div className="flex items-center bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 rounded-md p-1 focus-within:border-[#ea580c] transition-all h-[30px]">
                  <input type="number" step="1" value={Math.round(m.ratio * 100)} onChange={(e) => updateMeat(idx, 'ratio', parseFloat(e.target.value) / 100)} className="w-full bg-transparent font-bold text-xs text-stone-900 dark:text-white text-center outline-none" />
          ...
          <button onClick={addMeat} className="w-full py-3 border-2 border-dashed border-stone-300 dark:border-stone-600 text-stone-400 rounded-lg font-bold text-[9px] uppercase hover:border-[#ea580c] hover:text-[#ea580c] transition-all hover:bg-orange-50/50">
            <i className="fas fa-plus mr-1"></i> Adicionar
          </button>
        </div>
        <div className="grid grid-cols-2 gap-2 pt-3 border-t border-stone-100 dark:border-stone-700">
          <button onClick={onClose} className="w-full bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-300 py-2.5 rounded-lg font-bold uppercase tracking-widest text-[8px] hover:bg-stone-200 dark:hover:bg-stone-600 transition-colors">Cancelar</button>
...
const CostsModal = ({ results, prices, setPrices, costResults, sellingPrice, setSellingPrice, onClose }: any) => {
  ...
  return (
    <PanelBase title="Gestão & Lucro" icon="calculator-dollar" className="fixed inset-x-0 top-[64px] bottom-0 z-[50] flex items-start justify-center p-3 bg-stone-50/95 dark:bg-stone-900/95 backdrop-blur-sm overflow-y-auto animate-in slide-in-from-bottom-4 duration-300 no-print" rightAction={
      <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600 transition active:scale-90">
        <i className="fas fa-times text-xs"></i>
      </button>
    }>
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 gap-1.5">
            <div className="bg-stone-50 dark:bg-stone-700 p-2 rounded-lg border border-stone-100 dark:border-stone-600 flex items-center justify-between">
              <span className="text-[10px] font-black uppercase text-stone-900 dark:text-white italic">Gordura</span>
              <div className="flex items-center gap-1.5">
                <span className="text-[9px] font-black text-stone-400">R$</span>
                <input type="text" inputMode="decimal" defaultValue={prices['Gordura Animal']?.toString().replace('.', ',')} onBlur={(e) => updatePrice('Gordura Animal', e.target.value)} className="w-16 bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-1 rounded-md font-black text-center text-xs outline-none focus:border-emerald-500 transition-all" />
              </div>
            </div>
            {results.meats.map((m: any, idx: number) => (
              <div key={idx} className="bg-stone-50 dark:bg-stone-700 p-2 rounded-lg border border-stone-100 dark:border-stone-600 flex items-center justify-between">
                <span className="text-[10px] font-black uppercase text-stone-900 dark:text-white italic">{m.name}</span>
                <div className="flex items-center gap-1.5">
                  <span className="text-[9px] font-black text-stone-400">R$</span>
                  <input type="text" inputMode="decimal" defaultValue={prices[m.name]?.toString().replace('.', ',')} onBlur={(e) => updatePrice(m.name, e.target.value)} className="w-16 bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 p-1 rounded-md font-black text-center text-xs outline-none focus:border-emerald-500 transition-all" />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-3 pt-4 border-t border-stone-50 dark:border-stone-700">
          <p className="text-[10px] font-black text-stone-500 uppercase tracking-widest italic">Simulador de Venda (R$/un.)</p>
          <div className="bg-emerald-50 dark:bg-emerald-900/30 p-4 rounded-xl border border-emerald-100 dark:border-emerald-900 flex items-center justify-between shadow-sm">
            <span className="text-xs font-black uppercase text-emerald-900 dark:text-emerald-300 italic">Preço de Venda</span>
            <div className="flex items-center gap-2">
              ...
              }} className="w-24 bg-white dark:bg-stone-800 border border-emerald-200 dark:border-emerald-900 p-2.5 rounded-lg font-black text-center text-lg text-emerald-600 dark:text-emerald-300 outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all" />
            </div>
          </div>
        </div>
        ...
        <button onClick={onClose} className="w-full bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-300 py-4 rounded-xl font-black uppercase tracking-widest text-[9px] hover:bg-stone-200 dark:hover:bg-stone-600 transition-all">Fechar</button>
      </div>
    </PanelBase>
  );
};

const Suggestions = ({ suggestions, apply, onClose, isSearching, loadSuggestions, currentCategory, customSearchQuery, setCustomSearchQuery, favorites, toggleFavorite, onShare }: any) => {
  ...
  return (
    <PanelBase title={`Blends: ${currentCategory} `} icon="robot" className="fixed inset-x-0 top-[64px] bottom-0 z-[50] flex items-start justify-center p-3 bg-stone-50/95 dark:bg-stone-900/95 backdrop-blur-sm overflow-y-auto animate-in slide-in-from-bottom-4 duration-300 no-print" rightAction={
      <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600 transition active:scale-90">
        <i className="fas fa-times text-xs"></i>
      </button>
    }>
      <div className="space-y-6">
        <div className="space-y-3">
          <form onSubmit={handleCustomSearch} className="relative">
            <input type="text" placeholder="Pesquisar blend específico..." value={customSearchQuery} onChange={(e) => setCustomSearchQuery(e.target.value)} className="w-full bg-stone-50 dark:bg-stone-700 border border-stone-100 dark:border-stone-600 rounded-2xl py-4 pl-5 pr-14 text-[10px] font-bold focus:bg-white dark:focus:bg-stone-800 focus:ring-4 focus:ring-orange-500/5 outline-none transition-all text-stone-900 dark:text-white" />
          ...
          <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
            {BLEND_CATEGORIES.map(cat => (
              <button key={cat} onClick={() => loadSuggestions(cat)} className={`px - 4 py - 2 rounded - lg text - [8px] font - black uppercase flex - shrink - 0 transition - all ${currentCategory === cat ? 'bg-[#ea580c] text-white shadow-lg shadow-orange-200' : 'bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600'} `}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <div className="min-h-[200px]">
          {isSearching ? (
            <div className="py-12 flex flex-col items-center justify-center gap-4">
              <div className="w-10 h-10 border-4 border-orange-100 dark:border-orange-900/30 border-t-[#ea580c] rounded-full animate-spin"></div>
              <p className="text-[9px] font-black uppercase text-stone-500 dark:text-stone-300 tracking-widest animate-pulse italic">Consultando Redes Profissionais...</p>
            </div>
          ) : suggestions.length === 0 ? (
            <div className="py-12 flex flex-col items-center justify-center gap-3 text-center">
              <i className="fas fa-search-minus text-stone-200 dark:text-stone-700 text-4xl mb-1"></i>
              <h4 className="font-black text-[10px] uppercase text-stone-600 dark:text-stone-300 italic">Nenhum blend encontrado</h4>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {suggestions.map((s: any, idx: number) => (
                  <div key={idx} onClick={() => apply(s)} className="group bg-stone-50 dark:bg-stone-700 p-4 rounded-2xl border border-stone-100 dark:border-stone-600 hover:border-[#ea580c] hover:bg-white dark:hover:bg-stone-800 cursor-pointer transition-all hover:shadow-lg relative">
                    <button onClick={(e) => { e.stopPropagation(); toggleFavorite(s); }} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-white dark:bg-stone-800 flex items-center justify-center shadow-sm z-10 transition-all hover:scale-110">
                      <i className={`${favorites.some((f: any) => f.name === s.name) ? "fas text-red-500" : "far text-stone-300 dark:text-stone-500 hover:text-red-400"} fa - heart text - xs`}></i>
                    </button>
                    <div className="flex justify-between items-start mb-2 pr-8">
                      <h4 className="font-black text-[10px] text-stone-900 dark:text-white group-hover:text-[#ea580c] transition-colors leading-tight">{s.name}</h4>
                      <div className="flex gap-1">
                        <span className="text-[7px] font-black text-[#ea580c] bg-orange-50 dark:bg-orange-900/30 px-2 py-0.5 rounded-md">{(s.fatRatio * 100).toFixed(0)}% FAT</span>
                        <button onClick={(e) => { e.stopPropagation(); onShare(s); }} className="w-5 h-5 rounded bg-stone-100 dark:bg-stone-700 text-stone-400 dark:text-stone-300 hover:text-[#ea580c] transition-colors" title="Compartilhar">
                          <i className="fas fa-share-alt text-[8px]"></i>
                        </button>
                      </div>
                    </div>
                    <p className="text-[9px] text-stone-500 dark:text-stone-400 line-clamp-2 italic font-medium leading-relaxed">"{s.description}"</p>
                  </div>
                ))}
              </div>
              {allCitations.length > 0 && (
                <div className="pt-6 border-t border-stone-50 dark:border-stone-700">
                  <h5 className="text-[8px] font-black text-stone-500 dark:text-stone-300 uppercase tracking-widest mb-3 flex items-center gap-1.5 italic">
                    <i className="fas fa-link text-[#ea580c]"></i> Fontes e Referências
                  </h5>
                  <div className="flex flex-wrap gap-1.5">
                    {allCitations.map((c, i) => (
                      <a key={i} href={c.uri} target="_blank" rel="noopener noreferrer" className="text-[8px] font-bold text-stone-600 dark:text-stone-300 bg-stone-50 dark:bg-stone-700 px-3 py-1.5 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-600 transition-all flex items-center gap-1.5 border border-stone-100 dark:border-stone-600">
...
const Manual = ({ onClose }: any) => (
  <PanelBase title="Dicas de Elite" icon="lightbulb" className="fixed inset-x-0 top-[64px] bottom-0 z-[50] flex items-start justify-center p-3 bg-stone-50/95 dark:bg-stone-900/95 backdrop-blur-sm overflow-y-auto animate-in slide-in-from-bottom-4 duration-300 no-print" rightAction={
    <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600 transition active:scale-90">
      <i className="fas fa-times text-xs"></i>
    </button>
  }>
    <div className="space-y-6 text-xs font-medium text-stone-700 dark:text-stone-300 leading-relaxed">

      {/* Seção 1: Blend */}
      <section className="bg-amber-50 dark:bg-amber-900/30 p-4 rounded-2xl border border-amber-100 dark:border-amber-900">
        <h4 className="font-black text-[10px] uppercase text-amber-900 dark:text-amber-300 mb-2 flex items-center gap-1.5">
          <i className="fas fa-drumstick-bite"></i> 1. O Blend de Carnes
        </h4>
        <ul className="space-y-2 list-disc pl-4 text-[9px] text-stone-600 dark:text-stone-400">
      ...
      {/* Seção 2: Moagem */}
      <section className="bg-stone-50 dark:bg-stone-700 p-4 rounded-2xl border border-stone-100 dark:border-stone-600">
        <h4 className="font-black text-[10px] uppercase text-stone-900 dark:text-white mb-2 flex items-center gap-1.5">
      ...
      <section className="bg-amber-50 dark:bg-amber-900/30 p-4 rounded-2xl border border-amber-100 dark:border-amber-900">
        ...
        <p className="text-[9px] text-stone-600 dark:text-stone-400">
      ...
      <button onClick={onClose} className="w-full bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-300 py-4 rounded-xl font-black uppercase tracking-widest text-[9px]">Fechar</button>
    </div>
  </PanelBase>
);

const HistoryModal = ({ onClose, onOpenCare, onOpenDIY }: any) => (
  <PanelBase title="Enciclopédia Burger" icon="book-open" className="fixed inset-x-0 top-[64px] bottom-0 z-[50] flex items-start justify-center p-3 bg-stone-50/95 dark:bg-stone-900/95 backdrop-blur-sm overflow-y-auto animate-in slide-in-from-bottom-4 duration-300 no-print" rightAction={
    <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600 transition active:scale-90">
      <i className="fas fa-times text-xs"></i>
    </button>
  }>
    <div className="space-y-10 text-stone-700 dark:text-stone-300">
      <section className="space-y-4">
        <h4 className="font-black text-lg text-stone-900 dark:text-white uppercase italic flex items-center gap-2.5"><i className="fas fa-globe-americas text-[#ea580c]"></i> A Jornada Mundial</h4>
      ...
      <section className="space-y-4">
        <h4 className="font-black text-lg text-stone-900 dark:text-white uppercase italic flex items-center gap-2.5"><i className="fas fa-map-marker-alt text-[#ea580c]"></i> O Hambúrguer no Brasil</h4>
        <div className="space-y-3 text-xs leading-relaxed font-medium text-justify">
          <p>
      ...
      <section className="space-y-4">
        <h4 className="font-black text-lg text-stone-900 dark:text-white uppercase italic flex items-center gap-2.5"><i className="fas fa-fire text-[#ea580c]"></i> Métodos de Preparo & Tempos</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="bg-stone-50 dark:bg-stone-700 p-5 rounded-2xl border border-stone-100 dark:border-stone-600 hover:bg-white dark:hover:bg-stone-800 hover:shadow-lg transition-all">
            <div className="flex justify-between items-start mb-2">
              <h5 className="font-black text-[10px] uppercase text-stone-900 dark:text-white flex items-center gap-1.5"><i className="fas fa-burn text-orange-500"></i> Churrasqueira</h5>
              <span className="text-[8px] font-black bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-300 px-2 py-0.5 rounded-md">8-12 MIN</span>
            </div>
          ...
          <div className="bg-stone-50 dark:bg-stone-700 p-5 rounded-2xl border border-stone-100 dark:border-stone-600 hover:bg-white dark:hover:bg-stone-800 hover:shadow-lg transition-all">
            <div className="flex justify-between items-start mb-2">
              <h5 className="font-black text-[10px] uppercase text-stone-900 dark:text-white flex items-center gap-1.5"><i className="fas fa-square text-stone-400"></i> Chapa / Smash</h5>
              <span className="text-[8px] font-black bg-stone-200 dark:bg-stone-600 text-stone-600 dark:text-stone-300 px-2 py-0.5 rounded-md">3-6 MIN</span>
            </div>
          ...
          <div className="bg-stone-50 dark:bg-stone-700 p-5 rounded-2xl border border-stone-100 dark:border-stone-600 hover:bg-white dark:hover:bg-stone-800 hover:shadow-lg transition-all">
            <div className="flex justify-between items-start mb-2">
              <h5 className="font-black text-[10px] uppercase text-stone-900 dark:text-white flex items-center gap-1.5"><i className="fas fa-wind text-blue-400"></i> Air Fryer</h5>
              <span className="text-[8px] font-black bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 px-2 py-0.5 rounded-md">10-15 MIN</span>
            </div>
          ...
          <div className="bg-stone-50 dark:bg-stone-700 p-5 rounded-2xl border border-stone-100 dark:border-stone-600 hover:bg-white dark:hover:bg-stone-800 hover:shadow-lg transition-all">
            <div className="flex justify-between items-start mb-2">
              <h5 className="font-black text-[10px] uppercase text-stone-900 dark:text-white flex items-center gap-1.5"><i className="fas fa-door-closed text-stone-600"></i> Forno</h5>
              <span className="text-[8px] font-black bg-stone-200 dark:bg-stone-600 text-stone-600 dark:text-stone-300 px-2 py-0.5 rounded-md">15-20 MIN</span>
            </div>
      ...
      <section className="space-y-4">
        <h4 className="font-black text-lg text-stone-900 dark:text-white uppercase italic flex items-center gap-2.5"><i className="fas fa-utensils text-[#ea580c]"></i> Acompanhamentos de Mestre</h4>
        <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 pt-4">
        <button onClick={onOpenCare} className="bg-stone-100 dark:bg-stone-700 text-stone-900 dark:text-white p-4 rounded-xl font-black uppercase text-[8px] tracking-widest flex items-center justify-center gap-2 hover:bg-red-50 hover:text-red-600 transition-all border border-stone-200 dark:border-stone-600">
          <i className="fas fa-shield-heart text-sm"></i>
          Segurança
        </button>
        <button onClick={onOpenDIY} className="bg-stone-100 dark:bg-stone-700 text-stone-900 dark:text-white p-4 rounded-xl font-black uppercase text-[8px] tracking-widest flex items-center justify-center gap-2 hover:bg-stone-800 hover:text-white transition-all border border-stone-200 dark:border-stone-600">
...
const CareModal = ({ onClose }: any) => (
  <PanelBase title="Atenção Sanitária" icon="exclamation-triangle" className="fixed inset-x-0 top-[64px] bottom-0 z-[50] flex items-start justify-center p-3 bg-stone-50/95 dark:bg-stone-900/95 backdrop-blur-sm overflow-y-auto animate-in slide-in-from-bottom-4 duration-300 no-print" rightAction={
    <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600 transition active:scale-90">
      <i className="fas fa-times text-xs"></i>
    </button>
  }>
    <div className="space-y-4 text-xs font-medium text-stone-700 dark:text-stone-300 leading-relaxed">
      <p className="bg-red-50 dark:bg-red-900/30 p-5 rounded-2xl border border-red-100 dark:border-red-900 text-red-900 dark:text-red-300 font-bold italic">"Zona de Perigo: Entre 5°C e 60°C as bactérias se multiplicam rapidamente. Mantenha a carne sempre refrigerada."</p>
      ...
      <button onClick={onClose} className="w-full bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-300 py-4 rounded-xl font-black uppercase tracking-widest text-[9px] mt-2">Entendido</button>
    </div>
  </PanelBase>
);

const DIYModal = ({ onClose }: any) => (
  <PanelBase title="Equipamentos DIY" icon="tools" className="fixed inset-x-0 top-[64px] bottom-0 z-[50] flex items-start justify-center p-3 bg-stone-50/95 dark:bg-stone-900/95 backdrop-blur-sm overflow-y-auto animate-in slide-in-from-bottom-4 duration-300 no-print" rightAction={
    <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600 transition active:scale-90">
      <i className="fas fa-times text-xs"></i>
    </button>
  }>
    <div className="space-y-4 text-xs font-medium text-stone-700 dark:text-stone-300 leading-relaxed">
      <div className="space-y-3">
        <div className="bg-stone-50 dark:bg-stone-700 p-5 rounded-2xl border border-stone-100 dark:border-stone-600 flex gap-3 items-start">
          <i className="fas fa-cog text-stone-500 text-lg mt-0.5"></i>
          <div>
            <h5 className="font-black text-[9px] uppercase text-stone-900 dark:text-white mb-0.5">Moedor Improvisado</h5>
        ...
        <div className="bg-stone-50 dark:bg-stone-700 p-5 rounded-2xl border border-stone-100 dark:border-stone-600 flex gap-3 items-start">
          ...
          <div>
            <h5 className="font-black text-[9px] uppercase text-stone-900 dark:text-white mb-0.5">Aro de Moldar</h5>
        ...
        <div className="bg-stone-50 dark:bg-stone-700 p-5 rounded-2xl border border-stone-100 dark:border-stone-600 flex gap-3 items-start">
          ...
          <div>
            <h5 className="font-black text-[9px] uppercase text-stone-900 dark:text-white mb-0.5">Prensa Smash</h5>
            <p className="text-[10px]">Uma espátula rígida e um peso (como outra panela de fundo reto) funcionam perfeitamente.</p>
          </div>
        </div>
      </div>
      <button onClick={onClose} className="w-full bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-300 py-4 rounded-xl font-black uppercase tracking-widest text-[9px] mt-2">Fechar</button>
    </div>
  </PanelBase>
);
...
const InstallGuide = ({ onClose }: any) => (
  <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-fade-in" onClick={onClose}>
    <div className="bg-white dark:bg-stone-800 rounded-3xl p-8 max-w-sm w-full shadow-2xl relative" onClick={e => e.stopPropagation()}>
      ...
      <div className="text-center">
        <div className="w-16 h-16 bg-orange-100 dark:bg-orange-900/30 rounded-full flex items-center justify-center mx-auto mb-4 text-[#ea580c]">
          <i className="fas fa-download text-2xl"></i>
        </div>
        <h3 className="text-xl font-black text-stone-800 dark:text-white mb-2">Instalar App</h3>
        <p className="text-stone-600 dark:text-stone-300 mb-6 text-sm leading-relaxed">
          Instale o Burger Master para acesso offline e melhor performance.
        </p>

        <div className="bg-stone-50 dark:bg-stone-700/50 rounded-xl p-4 mb-6 text-left">
          <div className="flex items-start gap-3 mb-3">
            <span className="bg-stone-200 dark:bg-stone-600 text-stone-600 dark:text-stone-200 w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold shrink-0">1</span>
            <p className="text-xs text-stone-600 dark:text-stone-300">Toque no botão <span className="font-bold">Compartilhar</span> do seu navegador</p>
          </div>
          <div className="flex items-start gap-3">
            <span className="bg-stone-200 dark:bg-stone-600 text-stone-600 dark:text-stone-200 w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold shrink-0">2</span>
            <p className="text-xs text-stone-600 dark:text-stone-300">Selecione <span className="font-bold">Adicionar à Tela de Início</span></p>
          </div>
        </div>

        <button onClick={onClose} className="bg-[#ea580c] text-white px-8 py-3 rounded-xl font-bold uppercase text-xs tracking-wider w-full hover:bg-[#c2410c] transition-colors">
          Entendi
        </button>
      </div>
    </div>
...
const FavoritesList = ({ favorites, apply, remove, onClose }: any) => (
  <PanelBase title="Meus Favoritos" icon="heart" className="fixed inset-x-0 top-[64px] bottom-0 z-[50] flex items-start justify-center p-3 bg-stone-50/95 dark:bg-stone-900/95 backdrop-blur-sm overflow-y-auto animate-in slide-in-from-bottom-4 duration-300 no-print" rightAction={
    <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-600 transition active:scale-90">
      <i className="fas fa-times text-xs"></i>
    </button>
  }>
    {favorites.length === 0 ? (
      <div className="py-12 flex flex-col items-center justify-center gap-3 text-center">
        <i className="far fa-heart text-stone-200 dark:text-stone-700 text-4xl mb-1"></i>
        <h4 className="font-black text-[10px] uppercase text-stone-600 dark:text-stone-300 italic">Nenhum favorito salvo</h4>
        <p className="text-[9px] text-stone-400 dark:text-stone-500">Salve seus blends preferidos na aba Explorar.</p>
      </div>
    ) : (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {favorites.map((s: SuggestedBlend, idx: number) => (
          <div key={idx} className="group bg-stone-50 dark:bg-stone-700 p-4 rounded-2xl border border-stone-100 dark:border-stone-600 hover:border-red-500 hover:bg-white dark:hover:bg-stone-800 transition-all hover:shadow-lg relative">
            <button onClick={(e) => { e.stopPropagation(); remove(s); }} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-white dark:bg-stone-800 text-stone-300 dark:text-stone-500 hover:text-red-500 flex items-center justify-center shadow-sm z-10 transition-colors">
              <i className="fas fa-trash-alt text-xs"></i>
            </button>
            <div onClick={() => apply(s)} className="cursor-pointer">
              <div className="flex justify-between items-start mb-2 pr-8">
                <h4 className="font-black text-[10px] text-stone-900 dark:text-white group-hover:text-red-500 transition-colors leading-tight">{s.name}</h4>
              </div>
              <div className="flex gap-2 mb-2">
                <span className="text-[7px] font-black text-red-500 bg-red-50 dark:bg-red-900/30 px-2 py-0.5 rounded-md">{(s.fatRatio * 100).toFixed(0)}% FAT</span>
                <span className="text-[7px] font-black text-stone-400 dark:text-stone-500 bg-stone-100 dark:bg-stone-700 px-2 py-0.5 rounded-md">{new Date().toLocaleDateString()}</span>
              </div>
              <p className="text-[9px] text-stone-500 dark:text-stone-400 line-clamp-2 italic font-medium leading-relaxed">"{s.description}"</p>
            </div>
          </div>
...
const ShareModal = ({ blend, onClose }: any) => {
  ...
  const handleShare = async () => {
    ...
    try {
      const canvas = await html2canvas(cardRef.current, { scale: 2, backgroundColor: '#1c1917' });
      const image = canvas.toDataURL("image/png");

      // Create link to download
      const link = document.createElement('a');
      link.href = image;
      link.download = `BurgerMaster - Blend - ${blend.name.replace(/\s+/g, '-')}.png`;
      link.click();

      // Try native share if mobile
      if (navigator.share) {
        const blob = await (await fetch(image)).blob();
        await navigator.share({
          files: [new File([blob], 'blend.png', { type: 'image/png' })],
          title: 'Meu Blend Burger Master',
          text: `Confira este blend: ${blend.name} `
        });
      }
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-stone-800 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl animate-fade-in" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-stone-100 dark:border-stone-700 flex justify-between items-center bg-stone-50 dark:bg-stone-800">
          <h3 className="font-black text-stone-800 dark:text-white uppercase tracking-wider text-xs">Compartilhar Blend</h3>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-stone-200 dark:bg-stone-700 text-stone-500 dark:text-stone-300 hover:bg-stone-300 flex items-center justify-center transition-colors">
            <i className="fas fa-times"></i>
          </button>
        </div>

        <div className="p-6 bg-stone-100 dark:bg-stone-900 flex justify-center">
          {/* Card to Capture */}
          <div ref={cardRef} className="bg-[#1c1917] text-white p-6 rounded-2xl shadow-xl w-[280px] relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-[#ea580c] blur-[60px] opacity-40 rounded-full"></div>
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-4">
                <i className="fas fa-hamburger text-[#ea580c] text-xl"></i>
                <span className="font-black tracking-tighter text-lg">BURGER MASTER</span>
              </div>

              <h2 className="text-2xl font-black text-white leading-tight mb-2">{blend.name}</h2>
              <div className="inline-block bg-[#ea580c] text-white text-[10px] font-bold px-2 py-0.5 rounded mb-4">
                {(blend.fatRatio * 100).toFixed(0)}% GORDURA
              </div>

              <div className="space-y-2 mb-6">
                {blend.meats.map((m: any, idx: number) => (
                  <div key={idx} className="flex justify-between text-sm border-b border-stone-700 pb-1">
                    <span className="text-stone-300">{m.name}</span>
                    <span className="font-bold">{(m.ratio * 100).toFixed(0)}%</span>
                  </div>
                ))}
              </div>

              <p className="text-[9px] text-stone-500 text-center uppercase tracking-widest mt-4">Calculado com IA Profissional</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 bg-white dark:bg-stone-800 border-t border-stone-100 dark:border-stone-700">
          <button
            ...
            className="w-full bg-[#ea580c] text-white py-3 rounded-xl font-bold uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-[#c2410c] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-orange-200"
          >
            {isGenerating ? (
              <>
                <i className="fas fa-circle-notch animate-spin"></i> Gerando...
              </>
            ) : (
              <>
                <i className="fas fa-share-alt"></i> Baixar Card do Instagram
              </>
            )}
          </button>
        </div>
...
const CameraOptionsModal = ({ onClose, onUpload, onOpenWebcam }: { onClose: () => void, onUpload: (e: React.ChangeEvent<HTMLInputElement>) => void, onOpenWebcam: () => void }) => (
  <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
    <div className="bg-white dark:bg-stone-800 w-full max-w-xs rounded-[2.5rem] shadow-2xl p-8 space-y-6 animate-in fade-in zoom-in duration-300">
      <div className="text-center">
        <h3 className="font-black text-lg italic uppercase text-stone-900 dark:text-white">Nova Análise</h3>
        <p className="text-[9px] text-stone-500 dark:text-stone-300 font-black uppercase tracking-widest mt-1.5">Selecione a Origem</p>
      </div>
      <div className="grid grid-cols-1 gap-3">
        ...
        <label className="cursor-pointer bg-stone-50 dark:bg-stone-700 hover:bg-stone-100 dark:hover:bg-stone-600 text-stone-900 dark:text-white p-5 rounded-2xl flex flex-col items-center gap-2 transition-all active:scale-95 border border-stone-100 dark:border-stone-600">
      ...
      <button onClick={onClose} className="w-full py-1 text-[9px] font-black uppercase tracking-widest text-stone-400 dark:text-stone-500 hover:text-stone-600 dark:hover:text-stone-300 transition-colors">Cancelar</button>
    </div>
  </div>
);
...
const App: React.FC = () => {
  ...
  const [shareBlend, setShareBlend] = useState<SuggestedBlend | null>(null);
  ...
  const [darkMode, setDarkMode] = useState(() => {
    return localStorage.getItem('burger-master-theme') === 'dark';
  });

  useEffect(() => {
    if (darkMode) {
      document.documentElement.classList.add('dark');
      localStorage.setItem('burger-master-theme', 'dark');
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('burger-master-theme', 'light');
    }
  }, [darkMode]);
  ...
  return (
    <div className={`min-h-screen bg-stone-50 dark:bg-stone-900 pb-32 font-['Inter'] selection:bg-[#ea580c] selection:text-white transition-colors duration-300`}>
...
`}} />
      ...
      {/* HEADER */}
      <header className="fixed top-0 left-0 right-0 bg-white/80 dark:bg-stone-900/80 backdrop-blur-xl border-b border-stone-200 dark:border-stone-800 z-50 transition-colors duration-300">
        <div className="max-w-md mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-[#ea580c] to-[#c2410c] rounded-xl flex items-center justify-center shadow-lg shadow-orange-500/20">
              <i className="fas fa-hamburger text-white text-lg"></i>
            </div>
            <div>
              <h1 className="font-black text-base text-stone-900 dark:text-white leading-none tracking-tight">BURGER MASTER</h1>
              <span className="text-[10px] font-bold text-[#ea580c] tracking-widest uppercase">Professional AI</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-9 h-9 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400 hover:bg-stone-200 dark:hover:bg-stone-700 hover:text-[#ea580c] transition-colors flex items-center justify-center"
            >
              <i className={`fas fa-${darkMode ? 'sun' : 'moon'}`}></i>
            </button>
            <button
              onClick={() => setShowFavorites(!showFavorites)}
              className="w-9 h-9 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400 hover:bg-red-50 hover:text-red-500 transition-colors flex items-center justify-center relative"
            >
              <i className={`fas fa-heart ${favorites.length > 0 ? 'text-red-500' : ''}`}></i>
              {favorites.length > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[9px] font-bold flex items-center justify-center rounded-full border-2 border-white dark:border-stone-900">
                  {favorites.length}
                </span>
              )}
            </button>
      ...
      <main className="max-w-md mx-auto px-4 pt-24 space-y-6">
        <section className="space-y-1 no-print">
          <div className="flex justify-between items-center px-1">
            <h3 className="font-black text-stone-900 dark:text-white uppercase text-[9px] tracking-widest flex items-center gap-1.5 italic">
        ...
        {showSuggestions && (
          <Suggestions
            ...
            onShare={(s: SuggestedBlend) => setShareBlend(s)}
          />
        )}
        ...
        {shareBlend && <ShareModal blend={shareBlend} onClose={() => setShareBlend(null)} />}
