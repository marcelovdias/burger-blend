    ...
    <div className="min-h-screen bg-stone-50 pb-24">
      ...
      <div className="print-only p-8 max-w-[210mm] mx-auto bg-white text-black font-sans">
        <div className="border-b-4 border-black pb-4 mb-8 flex justify-between items-end">
          <div>
            <h1 className="text-4xl font-black uppercase tracking-tighter italic text-black">Burger Master Pro</h1>
            <p className="text-sm font-bold uppercase tracking-[0.3em] mt-1 text-stone-600">Ordem de Produção Técnica</p>
          </div>
          <div className="text-right">
            <div className="text-xs font-bold uppercase text-stone-500 mb-1">ID do Pedido</div>
            <div className="text-xl font-black font-mono bg-stone-100 px-2 py-1 inline-block">{Math.random().toString(36).substr(2, 9).toUpperCase()}</div>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-8 mb-8">
          <div className="bg-stone-50 p-4 border border-stone-200">
            <h2 className="text-xs font-black uppercase text-stone-500 mb-2 tracking-widest">Produto / Blend</h2>
            <div className="text-2xl font-black uppercase italic mb-1">{recipe.name}</div>
            <div className="flex gap-4 text-xs font-bold text-stone-600">
              <span><i className="fas fa-weight-hanging mr-1"></i> {recipe.unitWeight}g / unidade</span>
              <span><i className="fas fa-percentage mr-1"></i> {(recipe.fatRatio * 100).toFixed(0)}% Gordura</span>
            </div>
          </div>
          <div className="bg-stone-50 p-4 border border-stone-200">
            <h2 className="text-xs font-black uppercase text-stone-500 mb-2 tracking-widest">Meta de Produção</h2>
            <div className="flex justify-between items-end">
              <div>
                <div className="text-3xl font-black tabular-nums">{targetUnits} <span className="text-sm text-stone-500 font-bold uppercase">Unidades</span></div>
              </div>
              <div className="text-right">
                <div className="text-xs font-bold uppercase text-stone-500 mb-1">Peso Total (Massa)</div>
                <div className="text-xl font-black tabular-nums">{(results.total / 1000).toFixed(3)} kg</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8">
          <h2 className="text-sm font-black uppercase text-black border-b-2 border-black pb-2 mb-4 flex items-center gap-2">
            <i className="fas fa-layer-group"></i> Composição do Blend
          </h2>
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-stone-300">
                <th className="py-2 text-xs font-black uppercase text-stone-500 w-1/2">Componente</th>
                <th className="py-2 text-xs font-black uppercase text-stone-500 text-right">Proporção</th>
                <th className="py-2 text-xs font-black uppercase text-stone-500 text-right">Peso (Kg)</th>
              </tr>
            </thead>
            <tbody className="text-sm font-bold">
              <tr className="border-b border-stone-100">
                <td className="py-3 pl-2"><i className="fas fa-burn text-stone-400 mr-2"></i> Gordura Animal</td>
                <td className="py-3 text-right">{(recipe.fatRatio * 100).toFixed(0)}%</td>
                <td className="py-3 text-right tabular-nums bg-stone-50 pr-2">{(results.fat / 1000).toFixed(3)} kg</td>
              </tr>
              {results.meats.map((m, i) => (
                <tr key={i} className="border-b border-stone-100">
                  <td className="py-3 pl-2"><i className="fas fa-drumstick-bite text-stone-400 mr-2"></i> {m.name}</td>
                  <td className="py-3 text-right">{(m.ratioInTotal * 100).toFixed(1)}%</td>
                  <td className="py-3 text-right tabular-nums bg-stone-50 pr-2">{(m.weight / 1000).toFixed(3)} kg</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="bg-stone-100 border-t-2 border-stone-300">
                <td className="py-3 pl-2 font-black uppercase text-xs">Total</td>
                <td className="py-3 text-right font-black">100%</td>
                <td className="py-3 text-right font-black tabular-nums pr-2">{(results.total / 1000).toFixed(3)} kg</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            <h2 className="text-sm font-black uppercase text-black border-b-2 border-black pb-2 mb-4">
              <i className="fas fa-cog"></i> Processamento
            </h2>
            <div className="space-y-4">
              <div className="flex justify-between border-b border-stone-200 pb-2 border-dashed">
                <span className="text-xs font-bold text-stone-600">Moagem</span>
                <span className="text-xs font-black uppercase">{recipe.grindMethod || "Disco Médio (2x)"}</span>
              </div>
              <div className="flex justify-between border-b border-stone-200 pb-2 border-dashed">
                <span className="text-xs font-bold text-stone-600">Temperatura da Carne</span>
                <span className="text-xs font-black uppercase">0°C a 2°C</span>
              </div>
              <div className="flex justify-between border-b border-stone-200 pb-2 border-dashed">
                <span className="text-xs font-bold text-stone-600">Modelagem</span>
                <span className="text-xs font-black uppercase">{recipe.unitWeight}g / Aro 10-11cm</span>
              </div>
            </div>
          </div>
          ...
          <div>
            <h2 className="text-sm font-black uppercase text-black border-b-2 border-black pb-2 mb-4">
              <i className="fas fa-clipboard-check"></i> Controle
            </h2>
            <div className="border border-stone-300 h-32 p-2 relative">
              <span className="text-[10px] font-bold text-stone-400 uppercase absolute top-2 left-2">Observações / Lote</span>
            </div>
          </div>
        </div>

        <div className="mt-auto border-t-4 border-black pt-6 flex justify-between items-center">
          <div className="text-xs font-bold text-stone-500">
            <p>Solicitado para: <span className="inline-block w-32 border-b border-stone-300"></span></p>
            <p className="mt-2">Retirada: ____/____ às ____:____</p>
          </div>
          <div className="text-right">
            <div className="text-[10px] font-black uppercase tracking-widest text-stone-400">Assinatura do Responsável</div>
            <div className="h-10 border-b border-stone-300 w-48 ml-auto"></div>
          </div>
        </div>
      </div>
            ...
            <div key={idx} className="flex gap-2 items-center bg-stone-50 p-3 rounded-xl border border-stone-100">
              ...
              <div className="w-14 text-right">
                ...
                <div className="flex items-center justify-end gap-0.5">
                  <input type="number" step="1" value={Math.round(m.ratio * 100)} onChange={(e) => updateMeat(idx, 'ratio', parseFloat(e.target.value) / 100)} className="w-10 bg-transparent font-black text-xs text-stone-900 text-right outline-none" />
...
const Suggestions = ({ suggestions, apply, onClose, isSearching, loadSuggestions, currentCategory, customSearchQuery, setCustomSearchQuery }: any) => {
  const allCitations = useMemo(() => {
    ...
    suggestions.forEach((s: any) => s.citations?.forEach((c: any) => unique.set(c.uri, c)));
